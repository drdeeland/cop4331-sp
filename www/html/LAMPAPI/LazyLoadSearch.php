<?php
	...
	else
	{
		...
		$stmt = $conn->prepare("SELECT * FROM Contacts WHERE (Name LIKE ? OR Phone LIKE ? OR Email LIKE ?) AND UserID=? ORDER BY Name LIMIT ?, 10");
		$contactName = "%" . $search . "%";
		$stmt->bind_param("sssii", $contactName, $contactName, $contactName, $userID, $load);
		$stmt->execute();
